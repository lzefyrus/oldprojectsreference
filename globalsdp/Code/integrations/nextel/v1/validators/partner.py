...
from integrations.nextel.v1.services import partner as PartnerService
...
def validate_notification(type):
    ...
    def wrap(func):
        ...
        def wrapped_f(self, *args):
            ...
            # Validating configs
            try:
                host = self.application.settings['config']['nextel/v1/backend/host']['value']
                url_signature = self.application.settings['config']['nextel/v1/backend/url/notification/signature']['value']
                url_authentication = self.application.settings['config']['nextel/v1/backend/url/notification/authentication']['value']
                url_cancellation = self.application.settings['config']['nextel/v1/backend/url/notification/cancellation']['value']
                description_text = self.application.settings['config']['nextel/v1/backend/notification/message']['value']
                description_error_text = self.application.settings['config']['nextel/v1/backend/notification/error/message']['value']
...
def validate_mo(func):
    """
    Validates Nextel MO request (mandatory params only)
    """
    @wraps(func)
    def with_mo(self):
        ...
        try:
            configs = {
                'host': self.application.settings['config']['nextel/v1/backend/host'],
                'url': self.application.settings['config']['nextel/v1/backend/mo/url'],
                'ack': self.application.settings['config']['nextel/v1/mo/ack'],
                'is_billing': self.application.settings['config']['nextel/v1/mo/is_billing'],
                'keep_session': self.application.settings['config']['nextel/v1/mo/keep_session'],
                'description_code': self.application.settings['config']['nextel/v1/mo/description/code'],
                'description_text': self.application.settings['config']['nextel/v1/mo/description/text'],
                'fail_description_text': self.application.settings['config']['nextel/v1/mo/fail/description/text'],
            }
        except KeyError:
            message = "Could not find access configs for Nextel" \
                      "Operation Hash: {0}."\
                      .format(MO_HASH)
            log.error(message)
