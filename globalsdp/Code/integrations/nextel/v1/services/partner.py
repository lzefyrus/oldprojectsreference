...
##########Notification##########
################################
@celery.task(base=Configs, name="nextel.v1.partner.send_notification")
def notify(configs, headers, body, type):
    ...
    try:
        ...
        try:
            ...
            notify.apply_async(args=[configs, headers, body], queue='nextel.v1.partner.send_notification.error')
...
def get_signature_configs(settings):
    ...
    return {
        'host': settings['config']['nextel/v1/backend/host']['value'],
        'url': settings['config']['nextel/v1/backend/url/notification/signature']['value'],
        'description_text': settings['config']['nextel/v1/backend/notification/message']['value'],
        'description_error_text': settings['config']['nextel/v1/backend/notification/error/message']['value'],
    }
...
def get_cancellation_configs(settings):
    ...
    return {
        'host': settings['config']['nextel/v1/backend/host']['value'],
        'url': settings['config']['nextel/v1/backend/url/notification/cancellation']['value'],
        'description_text': settings['config']['nextel/v1/backend/notification/message']['value'],
        'description_error_text': settings['config']['nextel/v1/backend/notification/error/message']['value'],
    }
...
def get_authentication_configs(settings):
    ...
    return {
        'host': settings['config']['nextel/v1/backend/host']['value'],
        'url': settings['config']['nextel/v1/backend/url/notification/authentication']['value'],
        'description_text': settings['config']['nextel/v1/backend/notification/message']['value'],
        'description_error_text': settings['config']['nextel/v1/backend/notification/error/message']['value'],
    }
...
def get_notification_body(xml):
    ...
    :param xml: xml body received from Nextel
    :return: json
    """
    return {
...
def get_notification_response(text, json, ack, status_code):
    """
    Parses json to xml in order to respond Nextel's request
...
##########MO##########
######################
@celery.task(base=Configs, name="nextel.v1.partner.send_mo")
def send_mo(configs, headers, body):
    ...
    try:
        ...
        try:
            ...
            send_mo.apply_async(args=[configs, headers, body], queue='nextel.v1.partner.send_mo.error')
...
def get_mo_configs(settings):
    ...
    return {
        'host': settings['config']['nextel/v1/backend/host']['value'],
        'url': settings['config']['nextel/v1/backend/mo/url']['value'],
        'ack': settings['config']['nextel/v1/mo/ack']['value'],
        'is_billing': settings['config']['nextel/v1/mo/is_billing']['value'],
        'keep_session': settings['config']['nextel/v1/mo/keep_session']['value'],
        'description_code': settings['config']['nextel/v1/mo/description/code']['value'],
        'description_text': settings['config']['nextel/v1/mo/description/text']['value'],
        'fail_description_text': settings['config']['nextel/v1/mo/fail/description/text']['value'],
    }
...
def get_mo_body(xml):
    ...
    """
    return {
        "company_id": "{0}".format(xml.get('company_id')),
        "service_id": "{0}".format(xml.get('service_id')),
        "carrier_id": "{0}".format(xml.find('carrier_id').text),
        "dispatcher_id": "{0}".format(xml.find('dispatcher_id').text),
        "message_id": "{0}".format(xml.find('message_id').text),
        "application_id": "{0}".format(xml.find('application_id').text),
        "large_account": "{0}".format(xml.find('large_account').text),
        "source": "{0}".format(xml.find('source').text),
        "request_datetime": "{0}".format(xml.find('request_datetime').text),
        "text": "{0}".format(xml.find('text').text)
    }
