...
def is_handler_subclass(cls, classnames=("ViewHandler", "APIHandler", "RestDBAPIHandler")):
...
def get_routes(package, db):
    ...
    :rtype: [(url, RequestHandler), ... ]
    """
    main_urls = list(chain(*[get_module_routes(modname) for modname in
                             gen_submodule_names(package)]))

    return [(a[0], a[1], dict(db=db)) for a in main_urls]
