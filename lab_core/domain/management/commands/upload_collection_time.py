...
from domain.models import Laboratory, LaboratoryCollectionTime


class Command(BaseCommand):
    help = 'Import data into LaboratoryCollectionTime table'
    DAYS_OF_THE_WEEK = {
        "Seg": 1,
        "Ter": 2,
        "Qua": 3,
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            for row in sheet.rows:
                ...
                try:
                    loh = LaboratoryCollectionTime.objects.get(laboratory=lab.id, week_day=week_day)
                    loh.opens_at = opens_at
                    loh.closes_at = closes_at
                    loh.is_open = is_open
                    print("Updating existing collection time: {0} {1}".format(lab_external_id, week_day))
                except ObjectDoesNotExist:
                    print("Creating new collection time: {0} {1}".format(lab_external_id, week_day))
                    loh = LaboratoryCollectionTime(laboratory=lab, week_day=week_day, is_open=is_open,
