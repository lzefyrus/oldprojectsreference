...
class Funambol(Provisioner):
    ...
    def requestRest(self, method, urlRestBase, params):
        ...
        if r.status_code == 200:
            ...
            if 'error' in request:
                ...
                elif request['error']['code'] == 'PRO-1107':
                    self.set_status(self.appConfig['statusCode']['emailAlreadyExiste']['code'])
                    code = self.appConfig['statusCode']['emailAlreadyExiste']['code']
                    message = self.appConfig['statusCode']['emailAlreadyExiste']['message']
    ...
    # GET
    ##
    def deleteUnlink(self):

        if self.mainId and re.match("^[A-Za-z0-9@_\-.]*$", self.mainId) or self.subMainId and re.match("^[A-Za-z0-9@_\-.]*$", self.subMainId):
            self.myFields['userid'] = self.mainId
            self.myFields['deviceid'] = self.subMainId
        else:
            raise self.utils.BaseExceptionError(
                self.appConfig['statusCode']['invalidKeyE']['code'],
                self.appConfig['statusCode']['invalidKeyE']['message'])

        return self.requestRest('GET', '%sprofile?action=unlink-device' % (self.urlRestBase), self.myFields)
